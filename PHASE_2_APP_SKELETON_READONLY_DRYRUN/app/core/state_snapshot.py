...
def write_snapshot(output: Path | None = None) -> Path:
    ...
    target = output or (paths.raw_outputs_dir / "readonly_snapshot_latest.json")
    return save_json_inside_phase2(target, snapshot, paths)


def main() -> int:
    parser = argparse.ArgumentParser(description="Collect a Phase 2 read-only snapshot.")
