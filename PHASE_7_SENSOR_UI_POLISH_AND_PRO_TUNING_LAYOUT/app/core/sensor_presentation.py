...
class SensorPresentation:
    """Build Phase 7 display models from normalized telemetry.
    ...
    def _cpu_hero(self) -> dict[str, Any]:
        ...
        subtitle = self.headline.get("cpu_temp_name") or diag.get("warning") or "CPU temperature source unavailable"
        return {
            "key": "cpu",
            "title": "CPU",
            "value": temp,
            "value_display": self._compact_number(temp) if temp is not None else "temp unavailable",
            "unit": "C" if temp is not None else "",
            "subtitle": self._dedupe_subtitle(self._compact_number(temp), subtitle),
            "source": "LHM",
            "tone": self._temp_tone(temp),
            "progress": load,
            "history_key": "cpu_temp_c" if temp is not None else "cpu_load_percent",
            "chips": self._chips(
                [
                    ("Load", load, "%", None),
                    ("Power", power, "W", 1),
    ...
    def _cpu_panel(self) -> dict[str, Any]:
        diag = self._diagnostics().get("cpu_temperature", {})
        temp_rows = self._top_rows("cpu", "Temperature", 4)
        return {
            "key": "cpu",
            "title": "CPU",
            "subtitle": diag.get("summary") or "Processor telemetry from LibreHardwareMonitor.",
            ...
            "details_title": "CPU temperature candidates",
            "details": self._detail_rows(temp_rows, 6),
    ...
    def _diagnostics(self) -> dict[str, Any]:
        ...
        cpu["raw_cpu_sensors"] = [row for row in self.raw_sensors if row.get("category") == "cpu"]
    ...
    def _presentmon_subtitle(self) -> str:
        ...
        status = source.get("status") or source.get("error") or "not capturing"
