...
class DashboardTab(QWidget):
    def __init__(self, window) -> None:
        ...
        layout.addWidget(
            make_page_header(
                "AeroTune",
                "Read-only telemetry, backup status, power-plan visibility, and guarded CPU apply previews for the AERO X16 optimization workflow.",
                [("READ-ONLY / DRY-RUN", "safe"), ("Phase 5 CPU Apply Foundation", "neutral")],
            )
        )
        ...
        sensor_card, sensor_layout = make_card("Telemetry readiness", "Phase 6 sensor normalization status and latest read-only sensor health.")
