...
from app.core.config_loader import save_json_inside_phase8
...
class TelemetryTab(QWidget):
    def __init__(self, window) -> None:
        ...
        self._build_provider_status(layout)
    ...
    def _build_provider_status(self, layout: QVBoxLayout) -> None:
        self.provider_section, self.provider_layout = make_card(
            "Telemetry Providers",
            "Provider health is read-only and explains partial CPU data without treating hidden sensors as app failures.",
        )
        self.provider_section.setObjectName("sensor_provider_status_section")
        self.provider_grid = QGridLayout()
        self.provider_grid.setHorizontalSpacing(10)
        self.provider_grid.setVerticalSpacing(8)
        self.provider_layout.addLayout(self.provider_grid)
    ...
    def _build_raw_explorer(self, layout: QVBoxLayout) -> None:
        ...
        quick_panel = QWidget()
        quick_panel.setObjectName("sensor_quick_filter_chips")
        quick_row = QHBoxLayout(quick_panel)
        ...
        for label, value in [
            ...
            ("Stale zero", "stale_zero"),
            ("Selected", "selected"),
            ("Favorites", "favorites"),
        ]:
            button = QPushButton(label)
            button.setObjectName("sensor_quick_filter_button")
            button.clicked.connect(lambda _checked=False, v=value: self.apply_quick_filter(v))
            quick_row.addWidget(button)
        quick_row.addStretch(1)
        raw_layout.addWidget(quick_panel)
    ...
    def _build_cpu_diagnostics(self, layout: QVBoxLayout) -> None:
        ...
        export_cpu = QPushButton("Export CPU Diagnostics JSON")
        export_cpu.setObjectName("sensor_export_cpu_diagnostics_button")
        export_cpu.clicked.connect(self.export_cpu_diagnostics)
        raw_cpu_layout.addWidget(export_cpu)
    ...
    def _apply_refresh_payload(self, payload: dict[str, Any]) -> None:
        ...
        self.window.gpu_label.setText(headline.get("gpu_status_display", "GPU unavailable"))
        self.window.vram_label.setText(headline.get("vram_status_display", "VRAM n/a"))
        self.window.cpu_label.setText(headline.get("status_display", "CPU telemetry unavailable"))
        self.window.fps_label.setText(headline.get("fps_status_display", "FPS idle"))
        self._render_model(model)
        problem_sources = []
        for key in ("lhm", "nvidia", "presentmon"):
            value = payload.get(key, {})
            if not isinstance(value, dict):
                continue
            error = str(value.get("error") or "")
            if error and "No PresentMon CSV" not in error and "CSV exists but has no data" not in error:
                problem_sources.append(key)
        suffix = f" | source issue: {', '.join(problem_sources)}" if problem_sources else ""
    ...
    def _render_model(self, model: dict[str, Any]) -> None:
        ...
        self._render_provider_status(model)
    ...
    def refresh_raw_table(self) -> None:
        ...
        for row in self.current_raw_table_rows:
            ...
            searchable = " ".join(
                str(row.get(key, ""))
                for key in ["source", "provider", "hardware", "hardware_type", "sensor_type", "name", "category", "subcategory", "key", "validity", "validity_reason", "notes"]
            ).lower()
            ...
            if self.favorite_only.isChecked() and row.get("favorite") != "yes":
                continue
            if self.problem_only.isChecked() and raw.get("validity") == "valid":
                continue
            rows.append(row)
    ...
    def _render_cpu_diagnostics(self, diagnostics: dict[str, Any]) -> None:
        ...
        self.cpu_diag_summary.setText(
            "\n".join(
                [
                    ...
                    f"CPU load: {cpu.get('metric_health', {}).get('load', {}).get('validity', 'unknown')}",
                    f"CPU temp: {cpu.get('metric_health', {}).get('temperature', {}).get('validity', 'unknown')}",
                    f"CPU power: {cpu.get('metric_health', {}).get('power', {}).get('validity', 'unknown')}",
                    f"CPU clock: {cpu.get('metric_health', {}).get('clock', {}).get('validity', 'unknown')}",
                    f"CPU voltage: {cpu.get('metric_health', {}).get('voltage', {}).get('validity', 'unknown')}",
                    ...
                    f"Summary: {cpu.get('provider_summary') or cpu.get('summary') or 'none'}",
                    f"Recommendation: {cpu.get('provider_recommendation') or 'none'}",
                ]
            )
        )
        ...
        fill_table(
            self.cpu_raw_table,
            ["Source", "Hardware", "Type", "Name", "Value", "Unit", "Validity", "Validity reason", "Selected", "Notes"],
            [
                [
                    row.get("source"),
                    row.get("hardware"),
    ...
    def pin_selected_sensor(self) -> None:
        ...
        save_json_inside_phase8(self.window.paths.config_dir / "sensor_favorites.json", self.window.sensor_favorites, self.window.paths)
    ...
    def apply_quick_filter(self, value: str) -> None:
        self.clear_raw_filters()
        if value in ("cpu", "gpu", "memory", "network"):
            self.category_filter.setCurrentText(value)
        elif value == "invalid_value":
            self.problem_only.setChecked(True)
            self.raw_search.setText("invalid_value")
        elif value == "stale_zero":
            self.problem_only.setChecked(True)
            self.raw_search.setText("stale_zero")
        elif value == "selected":
            self.selected_only.setChecked(True)
        elif value == "favorites":
            self.favorite_only.setChecked(True)
        self.refresh_raw_table()

    def export_raw_snapshot(self) -> None:
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        path = self.window.paths.raw_outputs_dir / f"sensor_raw_snapshot_phase8_{timestamp}.json"
        data = {
            "generated_local": datetime.now().isoformat(timespec="seconds"),
            "count": len(self.current_raw_rows),
            "sensors": self.current_raw_rows,
        }
        save_json_inside_phase8(path, data, self.window.paths)
        QMessageBox.information(self, "Raw sensor snapshot exported", f"Saved app-side snapshot only:\n{path}")

    def export_cpu_diagnostics(self) -> None:
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        path = self.window.paths.raw_outputs_dir / f"cpu_diagnostics_{timestamp}.json"
        diagnostics = (self.current_presentation.get("diagnostics", {}) or {}).get("cpu_temperature", {})
        data = {
            "generated_local": datetime.now().isoformat(timespec="seconds"),
            "diagnostics": diagnostics,
            "raw_cpu_sensors": diagnostics.get("raw_cpu_sensors", []),
        }
        save_json_inside_phase8(path, data, self.window.paths)
        QMessageBox.information(self, "CPU diagnostics exported", f"Saved app-side diagnostics only:\n{path}")

    def _render_provider_status(self, model: dict[str, Any]) -> None:
        while self.provider_grid.count():
            item = self.provider_grid.takeAt(0)
            widget = item.widget()
            if widget:
                widget.deleteLater()
        try:
