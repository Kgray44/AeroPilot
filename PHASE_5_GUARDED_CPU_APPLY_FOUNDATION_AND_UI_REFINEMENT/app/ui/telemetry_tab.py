...
class TelemetryTab(QWidget):
    def __init__(self, window) -> None:
        ...
        self.presentmon_state_label = QLabel("PresentMon: stopped")
        self.presentmon_state_label.setObjectName("presentmon_running_state_label")
        pm_layout.addWidget(self.presentmon_state_label)
        ...
        self.lhm_headline_table = QTableWidget()
        lhm_layout.addWidget(self.lhm_headline_table)
    ...
    def refresh_all(self) -> None:
        ...
        lhm_headline = self.window.lhm.headline(getattr(self, "_last_lhm_snapshot", None)) if hasattr(self, "_last_lhm_snapshot") else {"cpu_temp_display": "Refresh LHM", "status_display": "Refresh LHM", "sensors_display": "not read"}
        cpu_headline = lhm_headline.get("cpu_temp_display") or lhm_headline.get("status_display", "n/a")
    ...
    def refresh_presentmon(self) -> dict:
        ...
        state = self.window.presentmon.state()
        self.presentmon_state_label.setText(
            f"PresentMon: {'running' if state.get('running') else 'stopped'} | output: {state.get('output_file') or 'none'}"
        )
    ...
    def refresh_lhm(self) -> dict:
        ...
        if not result.get("ok"):
            ...
            fill_table(self.lhm_headline_table, ["Field", "Value"], [["CPU", "telemetry unavailable"], ["Fan", "Fan unavailable"], ["Sensor count", 0]])
            return result
        rows = result.get("sensors", [])
        headline = self.window.lhm.headline(result)
        fill_table(
            self.lhm_headline_table,
            ["Field", "Value"],
            [
                ["Selected CPU temp sensor", headline.get("cpu_temp_name") or "unavailable"],
                ["Selected CPU temp value", headline.get("cpu_temp_display")],
                ["CPU load", headline.get("cpu_load_display") or "unavailable"],
                ["CPU power", headline.get("cpu_power_display") or "unavailable"],
                ["Fan RPM", headline.get("fan_display")],
                ["Total sensor count", headline.get("sensors_display")],
            ],
        )
        fill_table(
            self.lhm_table,
