...
class MainWindow(QMainWindow):
    ...
    def _apply_lhm_headline(self, headline: dict) -> None:
        self._lhm_busy = False
        if headline.get("ok"):
            self.cpu_label.setText(f"CPU {headline.get('CPU')} | {headline.get('Fan')} | {headline.get('Sensors')}")
        else:
            self.cpu_label.setText("CPU LHM unavailable")
